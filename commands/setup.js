module.exports = {
	...
	async execute(message) {
		...
		if(content.toLowerCase().startsWith(`${client.config.prefix}setup`)){
			...
			await channel.send('**Starting Vx_logs Setup!**\n*ETA: 30 seconds*').then(msg => {
				response = msg
			})
			ids = ''
			info = {
			...
			if(x){
				if(!guild.channels.cache.find(cc => cc.name === `📢・Vxngexnce-announcements`)){
					await guild.channels.create("📢・Vxngexnce-announcements", {
							type: 'GUILD_TEXT',
			...
			setTimeout(() => {
				response.edit(`**Starting Vx_logs Setup!**\n*ETA: 15 seconds*`)
			}, 15000)
			setTimeout(() => {
