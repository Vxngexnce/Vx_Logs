module.exports = {
	...
	async execute(message) {
		...
		if(content.toLowerCase().startsWith(`${client.config.prefix}hide`)){
			...
			let status = await GetResourceKvpString(`Vx_logs:${(args[2])}:${args[3]}`)
			if(status === "false" || status === null){
				status = 'true'
			} else {
				status = 'false'
			}
			await SetResourceKvp(`Vx_logs:${args[2].toLowerCase()}:${args[3].toLowerCase()}`, status)
...
\`${client.config.prefix}setup\` - Run the Vx_logsV1 Setup.
\`${client.config.prefix}uninstall\` - Remove all channels used in Vx_logsV1.
