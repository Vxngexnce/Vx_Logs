...
CreateThread(function() --[[ Permissions check for buildin DiscordAcePerms ]]
	...
	if not IsPrincipalAceAllowed('resource.' .. GetCurrentResourceName(), 'command.add_principal') then
		print('^1Error:^0 Vx_logs needs to have access to ^2add_principal^0 for the discord permissions to work.')
		add_principal = false
	end
	if not IsPrincipalAceAllowed('resource.' .. GetCurrentResourceName(), 'command.remove_principal') then
		print('^1Error:^0 Vx_logs needs to have access to ^2remove_principal^0 for the discord permissions to work.')
		remove_principal = false
	end
	if not IsPrincipalAceAllowed('resource.' .. GetCurrentResourceName(), 'command.add_ace') then
		print('^1Error:^0 Vx_logs needs to have access to ^2add_ace^0 for the discord permissions to work.')
		addd_ace = false
	end
	if not IsPrincipalAceAllowed('resource.' .. GetCurrentResourceName(), 'command.remove_ace') then
		print('^1Error:^0 Vx_logs needs to have access to ^2remove_ace^0 for the discord permissions to work.')
		remove_ace = false
	end
...
RegisterNetEvent("Vxngexnce:Vx_logsV1:GetConfigSettings")
AddEventHandler("Vxngexnce:Vx_logsV1:GetConfigSettings", function()
	if Config ~= nil then
		data = {
		...
		TriggerClientEvent("Vxngexnce:Vx_logsV1:SendConfigSettings", source, data)
	end
end)

RegisterServerEvent('Vxngexnce:Vx_logsV1:playerShotWeapon') --[[ Shooting logs. ]]
AddEventHandler('Vxngexnce:Vx_logsV1:playerShotWeapon', function(weapon, count)
	if not weapon then
...
RegisterServerEvent('Vxngexnce:Vx_logsV1:playerDied')
AddEventHandler('Vxngexnce:Vx_logsV1:playerDied',function(args)
	if args.kil == 0 then
...
RegisterServerEvent('Vxngexnce:Vx_logsV1:PlayerDamage') --[[ Damaghe Logs. ]]
AddEventHandler('Vxngexnce:Vx_logsV1:PlayerDamage', function(args)
...
RegisterServerEvent('Vxngexnce:Vx_logsV1:ClientDiscord')
AddEventHandler('Vxngexnce:Vx_logsV1:ClientDiscord', function(args)
...
RegisterNetEvent("Vxngexnce:Vx_logsV1:ScreenshotCB") --[[ Returning screenshot value. ]]
AddEventHandler("Vxngexnce:Vx_logsV1:ScreenshotCB", function(args)
	ServerFunc.CreateLog(args)
end)
...
-------------------------------------------------------^0]]):format(rv.version, rv.changelog))
				ServerFunc.CreateLog({ EmbedMessage = "**Vx_logsV1 Update V"..rv.version.."**\nDownload the latest update of Vx_logsV1"..rv.changelog..'\n\n**How to update?**\n1. Download the latest version.\n2. Replace all files with your old once **EXCEPT THE CONFIG** folder.\n3. run the `!vxlogs setup` command again and you\'re done.', channel = 'system'})
			end
		end
	end, 'GET')
end)
