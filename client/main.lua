...
CreateThread(function()
    while Config == nil do
        TriggerServerEvent("Vxngexnce:Vx_logsV1:GetConfigSettings") --[[ On client load this will make a request to the server to pull some config data. ]]
        Wait(0)
    end
    while lang == nil do
        if Config ~= nil then
...
RegisterNetEvent("Vxngexnce:Vx_logsV1:SendConfigSettings")
AddEventHandler("Vxngexnce:Vx_logsV1:SendConfigSettings", function(data)
...
exports('discord', function(msg, player_1, player_2, color, channel) --[[ This is to support the export from v1. ]]
	...
	TriggerServerEvent('Vxngexnce:Vx_logsV1:ClientDiscord', args)
end)
        ...
        elseif not IsPedShooting(playerped) and fireCount ~= 0 and timeout ~= 0 then --[[ When they player is finished shooring or the timeout has expires we will make the log. ]]
            ...
            if fireCount ~= 0 and timeout == 0 then --[[ When the timer is 0 and the weapon fire count is higher than 0 we will make a log. ]]
                if not ClientTables.WeaponNames[tostring(fireWeapon)] then --[[ Weapon info not found. creating log without weapon info. ]]
                    TriggerServerEvent('Vxngexnce:Vx_logsV1:playerShotWeapon', lang.shooting.undefined)
                    return
                end
                isLoggedWeapon = true
                ...
                if isLoggedWeapon then --[[ Sending the log with weapon info. ]]
                    TriggerServerEvent('Vxngexnce:Vx_logsV1:playerShotWeapon', ClientTables.WeaponNames[tostring(fireWeapon)], fireCount)
                end
                fireCount = 0
            end
        end
...
CreateThread(function()
	...
	while Config.damageLog do --[[ Checking if damage logs is enabled. ]]
		...
		if health > GetEntityHealth(PlayerPedId()) then
			...
			TriggerServerEvent('Vxngexnce:Vx_logsV1:PlayerDamage', math.floor((health - newHealth) / 2))
			health = newHealth
			Wait(1000)
		else
			Wait(1000)
	...
	while Config.deathLog do
		...
		if IsEntityDead(iPed) then
			if not hasRun then
				...
				TriggerServerEvent('Vxngexnce:Vx_logsV1:playerDied', { ['rsn'] = DeathReason, ['kil'] = GetPlayerServerId(killer) })
			end
		else
			Wait(500)
			hasRun = false
